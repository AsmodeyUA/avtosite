		...
		if (Data3007.open(myLog)) {
			...
			for (int i = 0; i < product.getMax_id(); i++) {
				...
				try {
					...
					File F = new File(config.pathIMGTecdocJ + '\\' + folder_name);
					fList = F.listFiles();
					...
					if (fList.length != 0) {
						for (int j = 0; j < fList.length; j++) {
							...
							if (fList[j].isFile()) {
								...
								//prod_Image[prod_Image_id].pathFileO = config.pathIMGTecdoc
								//		+ '\\' + folder_name + '\\' + path1;
								//prod_Image[prod_Image_id].pathFileN = config.pathIMGTecdocJ
								//		+ '\\' + folder_name + '\\' + path1;
					...
					if (fList.length != 0) {
						for (int j = 0; j < fList.length; j++) {
							...
							if (fList[j].isFile()) {
								...
								//prod_Image[prod_Image_id].pathFileO = "";
								//prod_Image[prod_Image_id].pathFileN = "";
								prod_Image[prod_Image_id].pathFileO=config.pathIMG + '\\'+folder_name+'\\'+path1;
								prod_Image[prod_Image_id].pathFileN=config.pathIMGJ + '\\'+folder_name+'\\'+path1;
								// System.out.println(prod_Image[prod_Image_id].toSql1String());
								if (NotFoundMainPic) {
									prod[i].setImage(pathTemp);
									NotFoundMainPic=false;
